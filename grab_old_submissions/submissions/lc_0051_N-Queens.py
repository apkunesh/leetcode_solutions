from typing import List
from copy import deepcopy

class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        row = []
        for j in range(n):
            row.append(set())
        hits = [deepcopy(row) for i in range(n)]
        output_line = "."*n
        candidate = [output_line for i in range(n)]
        solution = []
        def add_queen(i,j,hits):
            candidate[i] = candidate[i][0:j]+ "Q" + candidate[i][j+1:] if j<n-1 else candidate[i][0:j]+ "Q" 
            for i_iter in range(i+1,n): # Only update rows below
                x_diff = i_iter-i
                l_diag,center,r_diag = j - x_diff,j,j+x_diff
                if l_diag > -1:
                    hits[i_iter][l_diag].add(i)
                hits[i_iter][center].add(i)
                if r_diag < n:
                    hits[i_iter][r_diag].add(i)
        def remove_queen(i,j,hits):
            candidate[i] =  candidate[i][0:j]+ "." + candidate[i][j+1:] if j<n-1 else  candidate[i][0:j]+ "." 
            for i_iter in range(i+1,n):
                x_diff = i_iter-i
                l_diag,center,r_diag = j - x_diff,j,j+x_diff
                if l_diag > -1:
                    hits[i_iter][l_diag].remove(i)
                hits[i_iter][center].remove(i)
                if r_diag < n:
                    hits[i_iter][r_diag].remove(i)
        def recurse(hits,i,j):
            if hits[i][j] != set():
                return "bad"
            if i == n-1:
                if add_queen(i,j,hits) != "bad":
                    solution.append(deepcopy(candidate))
                    remove_queen(i,j,hits)
            else:
                if add_queen(i,j,hits) != "bad":                
                    for k in range(n):
                        recurse(hits,i+1,k)
                    remove_queen(i,j,hits)
        for j in range(n):
           recurse(hits,0,j)
        return solution
