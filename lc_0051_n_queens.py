...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        def add_queen(i,j,hits):
            candidate[i] = output_line[i][0:j]+ "Q" + output_line[j+1:] if j<n-1 else  output_line[i][0:j]+ "Q" 
        ...
        def remove_queen(i,j,hits):
            candidate[i] = output_line[i][0:j]+ "." + output_line[j+1:] if j<n-1 else  output_line[i][0:j]+ "." 
